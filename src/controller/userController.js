...
const userRegistration = async function(req, res) {
    try {
        ...
        //using destructuring
        // const { firstName, lastName, emailID, phoneNumber, password } = requestBody;
        ...
        if (!Validator.isValidInputValue(nickName)) {
            return res.status(400).send({
                status: false,
        ...
        if (!Validator.isValidOnlyCharacters(nickName)) {
            return res.status(400).send({
                status: false,
    ...
    //     }
        ...
        const isUniqueEmailID = await userModel.findOne({ nickName });

        if (isUniqueEmailID) {
            return res
                .status(400)
                .send({ status: false, message: "nickName  already exist" });
        }
...
const userLogin = async function(req, res) {
    try {
        ...
        const userName = requestBody.nickName;
        ...
        const userDetails = await userModel.findOne({ nickName: userName });
